...
public partial class WeekdayPlanViewModel : ObservableObject
{
    private readonly IWeeklyPlanService _planService;

    public WeekdayPlanViewModel(IWeeklyPlanService planService)
    {
        _planService = planService;
        _ = LoadAsync();
    }

    public ObservableCollection<WeekdayPlanRowViewModel> Rows { get; } = new();
    ...
    private async Task SaveAsync()
    {
        try
        {
            ...
            await _planService.SaveWeekdayAsync(Rows.ToList()).ConfigureAwait(true);
    ...
    private async Task LoadAsync()
    {
        try
        {
            var rows = await _planService.GetWeekdayRowsAsync().ConfigureAwait(true);
            Rows.Clear();
